...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mRN')
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}RN{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h} IP : {ip}')
	print('')
	cetak('[bold green] 1. FRIENDLIST CLONING\n 2. FILE CLONING\n 3. CHECK RESULTS\n 0. EXIT [bold green]')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[RN]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
